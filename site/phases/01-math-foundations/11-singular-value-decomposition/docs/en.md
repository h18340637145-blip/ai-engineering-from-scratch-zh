...
这意味着你在第 10 课中学到的所有关于降维的知识实际上都是在使用 SVD（奇异值分解）。PCA（主成分分析）是机器学习中 SVD 最常见的应用。
...
思路：为了找到最大的奇异值及其对应的向量，对 A^T A（或 A A^T）使用幂迭代。然后对矩阵进行降维，并重复该过程以找到下一个奇异值。
...
`code/svd.jl` 中的 Julia 版本使用 Julia 本机的 `svd()` 函数和 `LinearAlgebra` 包来演示相同的概念。
