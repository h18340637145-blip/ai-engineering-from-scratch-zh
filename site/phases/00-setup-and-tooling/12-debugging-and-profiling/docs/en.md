...
打印调试常常被忽视。它不应该被忽视。对于张量代码来说，有针对性的打印语句比通过调试器逐步执行更有效，因为你需要一次性看到形状、数据类型和值范围。```python
...
内置调试器在AI工作中被低估了。将`breakpoint()`插入训练循环中，可以交互式地检查张量。```python
...
当调试需求超出快速检查时，用日志记录替代print语句。```python
import logging
...
这显示了按累计时间排序的每个函数调用。对于逐行分析：```bash
...
#### 使用 tracemalloc 的 CPU 内存```python
import tracemalloc
...
for stat in top_stats[:10]:
    print(stat)
```#### 使用 memory_profiler 的 CPU 内存```bash
...
最频繁出现的错误。张量的形状为 `[batch, features]`，而模型期望的是 `[batch, channels, height, width]`。```python
...
你的模型在测试集上达到了 99% 的准确率。听起来很棒。其实这是一个错误。```python
...
不同设备（CPU与GPU）上的张量会导致运行时错误。但有时一个张量会静默地留在CPU上，而其他所有内容都在GPU上，导致训练过程变得非常缓慢。```python
...
TensorBoard展示了训练过程中随时间变化的情况。```bash
...
为了进行交互式调试，请使用 `launch.json` 配置 VS Code：```json
{
    "version": "0.2.0",
    "configurations": [
        {
