...
// Last built: 2026-08-04T11:38:31.123Z
    ...
    "lessons": [
      {
        ...
        "keywords": "步骤 1：系统基础 · Step 3: Node.js with pnpm · Step 4: Rust · Step 5: Julia (Optional) · Step 6: GPU Setup (If You Have One) · 第7步：验证所有内容"
      },
      {
        ...
        "keywords": "步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git · 步骤1：配置git"
      },
      {
        ...
        "keywords": "选项 1：本地 NVIDIA GPU · 选项 1：本地 NVIDIA GPU · 选项 3：云 GPU"
      },
      {
        "name": "APIs & Keys",
        "status": "complete",
        ...
        "keywords": "步骤1：安全存储API密钥 · Step 2: First API call (Python) · 步骤3：首次 API 调用（TypeScript）```typescript · Step 4: Raw HTTP (no SDK)"
      },
      {
        ...
        "keywords": "第一步：选择你的界面 · 步骤 3：单元格类型 · 第4步：魔术命令 · 第4步：魔术命令 · 笔记本 vs 脚本：何时使用哪种 · 常见陷阱"
      },
      {
        ...
        "keywords": "选项 1: uv venv（推荐） · 选项3：conda（当您需要时） · 本课程策略：分阶段策略 · 1. 全局安装 /no_think · 1. 全局安装```bash · 3. 忘记激活环境 /no_think · 3. 忘记激活环境```bash · 5. CUDA版本不匹配```bash"
      },
      {
        "name": "Docker for AI",
        "status": "complete",
        ...
        "keywords": "关键术语 · AI中常见的容器模式``` · 步骤 1：安装 Docker```bash · 第3步：了解基础镜像 · 第6步：使用Docker Compose构建多服务AI应用 · 第6步：使用Docker Compose构建多服务AI应用 · 没有GPU？"
      },
      {
        "name": "Editor Setup",
        "status": "complete",
      ...
      },
      {
        ...
        "keywords": "步骤1：了解你的shell · 步骤3：后台进程 · 第4步：tmux · 步骤6：使用SSH连接远程GPU服务器 · 第7步：AI工作的有用别名 · 步骤8: 常见AI终端模式"
      },
      {
        "name": "Linux for AI",
        "status": "complete",
      ...
      },
      {
        ...
        "keywords": "第1部分：打印调试（是的，它有效） · 第二部分：Python调试器（pdb和breakpoint） · 第3部分：Python日志记录 · 第4部分：代码部分的计时 · 第5部分：cProfile和line_profiler · 第7部分：常见AI错误及其捕捉方法 · 第9部分：VS Code 调试器 · 第9部分：VS Code 调试器"
      }
    ]
  },
  {
    ...
    "lessons": [
      {
        ...
        "keywords": "向量是点（和方向） · 矩阵是变换 · 点积衡量相似性 · 线性无关 · 基和秩 · 投影 · Gram-Schmidt 过程 · 第一步：从零开始构建向量（Python）```python"
      },
      {
        ...
        "keywords": "向量：有序的数字列表 · 矩阵：数字的网格 · 为什么形状很重要 · 操作映射 · 按元素乘法与矩阵乘法 · 广播 · 第一步：向量类```python"
      },
      {
        ...
        "keywords": "变换作为矩阵 · 反射 · 组合：变换的串联 · 特征值和特征向量 · 特征分解 · 行列式作为体积缩放因子 · 第一步：从零开始构建变换矩阵（Python）```python"
      },
      {
        ...
        "keywords": "什么是导数？ · 偏导数：一次一个变量 · 梯度：所有偏导数的向量 · 与优化的联系 · 数值导数与解析导数 · 手动计算简单函数的导数 · Hessian 矩阵 · 泰勒级数近似 · 机器学习中的积分 · 机器学习中的积分 · 机器学习中的积分 · 机器学习中的积分 · 机器学习中的积分 · 机器学习中的积分 · 机器学习中的积分 · 机器学习中的积分 · 机器学习中的积分 · 机器学习中的积分 · 机器学习中的积分 · 机器学习中的积分 · 机器学习中的积分 · 计算图中的多变量链式法则 · 雅可比矩阵 · 为什么这对神经网络很重要 · 第一步：从零开始构建数值导数```python · 步骤 2：偏导数和梯度```python · 步骤 4：在二维函数上进行梯度下降```python · 步骤 7：泰勒近似的应用```python · 步骤 8：这对神经网络的意义```python"
      },
      {
        ...
        "keywords": "链式法则 · 计算图 · 前向模式与反向模式 · 正向模式的双重数 · 构建一个自动求导引擎 · PyTorch 自动求导的内部工作原理 · 步骤 1：Value 类```python · 第二步：带梯度追踪的算术运算```python · 第三步：反向传播```python · 步骤 4：为完整引擎添加更多操作 · 步骤 5：从零开始构建小型 MLP · 第 6 步：梯度检查 · 第7步：与手动计算进行验证```python · 与 PyTorch 对比验证```python · 更复杂的表达式```python"
      },
      {
        ...
        "keywords": "事件、样本空间和概率 · 条件概率与独立性 · 概率质量函数 vs 概率密度函数 · 常见分布 · 联合分布与边缘分布 · 为什么正态分布无处不在 · 对数概率 · Softmax 作为概率分布 · 采样 · 第一步：概率基础```python"
      },
      {
        ...
        "keywords": "从联合概率到贝叶斯 · 四个部分 · 垃圾邮件过滤器示例 · 朴素贝叶斯：独立性假设 · 最大似然估计（MLE） · 最大后验概率（MAP） · 贝叶斯与频率学派：实际差异 · 为什么贝叶斯思维对机器学习很重要 · 步骤 1：贝叶斯定理函数```python · 步骤 3：在垃圾邮件数据上进行训练```python · 共轭先验 · 顺序贝叶斯更新 · 与 A/B 测试的联系"
      },
      {
        ...
        "keywords": "优化的含义 · SGD 与批量（batch）与小批量（mini-batch） · 动量：滚动下坡的球 · Adam：自适应学习率 · 学习率调度 · 凸函数与非凸函数 · 损失景观可视化 · 步骤 1：定义一个测试函数"
      },
      {
        ...
        "keywords": "信息量（惊讶度） · 熵（平均惊讶度） · 交叉熵（你每天使用的损失函数） · KL 散度（分布之间的距离） · 互信息 · 条件熵 · 联合熵 · 互信息量度（深入探讨） · 标签平滑和交叉熵 · 为什么交叉熵是分类损失 · 位（Bits）与自然对数单位（Nats） · 混淆度（Perplexity） · 第一步：信息内容和熵```python"
      },
      {
        ...
        "keywords": "维数灾难 · PCA：寻找重要的方向 · 解释的方差比例 · 选择成分数量 · t-SNE：保留邻域 · UMAP：更快，更好的全局结构 · 何时使用哪种方法 · 核心 PCA · 重建误差 · 步骤 1：从零开始实现 PCA```python"
      },
      {
        ...
        "keywords": "完整的分解 · 左奇异向量、奇异值、右奇异向量 · 外积形式 · 截断 SVD：低秩近似 · 使用SVD进行图像压缩 · 推荐系统中的SVD · NLP中的SVD：潜在语义分析 · 使用 SVD 进行降噪 · 通过SVD求伪逆 · 与 PCA 的联系 · 步骤 1：使用幂迭代从零开始构建 SVD"
      },
      {
        ...
        "keywords": "什么是张量 · 深度学习中的张量形状 · 内存布局是如何工作的 · 广播规则 · 步骤 1：张量存储和步长 · 步骤 2：重塑、压缩、扩展 · 第 4 步：逐元素操作和归约操作 · 步骤 5：使用 NumPy 进行广播 · 第6步：Einsum 操作 · 步骤 7：通过 einsum 实现注意力机制 · 从零开始 vs NumPy · 从零开始 vs PyTorch```python · 每个神经网络层都作为张量操作"
      },
      {
        ...
        "keywords": "IEEE 754：计算机如何存储实数 · 为什么 0.1 + 0.2 != 0.3 · 灾难性抵消及其规避 · 溢出和下溢 · Log-Sum-Exp 极值稳定算法 · 为什么 Softmax 需要最大值减法技巧 · NaN 和 Inf：检测与预防 · 数值梯度检查 · 混合精度训练 · bfloat16 与 float16：为什么 bfloat16 更适合训练``` · 梯度裁剪 · 归一化层作为数值稳定器 · 常见机器学习数值错误 · 步骤 1：演示浮点数精度限制```python · 混合精度模拟```python"
      },
      {
        ...
        "keywords": "范数：衡量向量大小 · L1 范数（曼哈顿距离） · L2 范数（欧几里得距离） · 余弦相似度和余弦距离 · 点积相似性 vs 余弦相似性 · 马氏距离 · Jaccard 相似度（用于集合） · 编辑距离（Levenshtein 距离） · KL散度（不是距离，但被当作距离使用） · Wasserstein距离（Earth Mover's Distance） · 为什么不同任务需要不同的距离度量 · 与损失函数的联系 · 最近邻搜索 · 步骤 1：所有规范和距离函数 · 步骤 2：相同数据，不同距离，不同邻居 · 步骤 3：嵌入相似性搜索"
      },
      {
        ...
        "keywords": "描述性统计：总结你的数据 · 相关性：变量如何共同变化 · 协方差矩阵 · 假设检验 · t检验 · 卡方检验 · 多重比较问题 · 自举方法（Bootstrap Methods） · 参数检验与非参数检验 · 中心极限定理：实际影响"
      },
      {
        ...
        "keywords": "为什么采样很重要 · 均匀随机采样 · 反向累积分布函数方法（反向变换采样） · 拒绝抽样 · 重要性采样 · Gibbs 抽样 · 温度采样（用于大语言模型） · Top-k 采样 · Top-p（核）采样 · 重参数化技巧（用于 VAEs） · Gumbel-Softmax（可微分的分类采样） · 分层抽样 · 与扩散模型的联系 · 步骤 1：均匀分布和逆累积分布函数采样```python · 步骤 2：拒绝采样```python · 步骤 3：重要性采样```python · 步骤 4：蒙特卡洛方法估计 pi```python · 步骤 6：Gibbs 采样```python · 第 8 步：Top-k 和 top-p 采样```python · 第 10 步：Gumbel-Softmax```python"
      },
      {
        "name": "Linear Systems",
        "status": "complete",
        ...
        "keywords": "Ax = b 几何上的含义 · 列图像与行图像 · 高斯消元法 · 部分选主元：为什么它很重要 · LU 分解 · QR 分解 · 乔列斯基分解 · 最小二乘法：当 Ax = b 没有精确解时 · 伪逆（Moore-Penrose） · 条件数 · 迭代方法：共轭梯度法 · 全貌：何时使用哪种方法 · 与机器学习的联系 · 步骤 1：带部分选主元的高斯消去法```python"
      },
      {
        ...
        "keywords": "凸集 · 凸函数 · 凸性的测试 · 为什么凸性很重要 · 机器学习中的凸与非凸 · 海森矩阵 · 牛顿法 · 有约束的优化 · KKT 条件 · 正则化作为约束优化 · 对偶性 · 实践中的二阶方法 · 步骤 1：凸性检查器"
      },
      {
        ...
        "keywords": "什么是复数？ · 复数运算 · 复数平面 · 极坐标形式 · 欧拉公式 · 为什么欧拉公式对机器学习很重要 · 与二维旋转的联系 · 单位根 · 与 DFT 的联系 · 为什么 i 不是虚数 · 复数指数与三角函数 · 与 Transformer 的联系 · 第一步：复数类 · 步骤 3：旋转 · 步骤 4：从复数运算进行 DFT```python · 步骤 5：逆 DFT · 步骤6：单位根```python"
      },
      {
        ...
        "keywords": "DFT 的定义 · 每个系数的含义 · 逆 DFT · FFT：使其快速 · 频谱分析 · 卷积定理 · DFT 属性 · 与位置编码的联系 · Alternative positional encodings · Benefits of positional encodings · Summary · 与卷积神经网络（CNN）的联系 · 频谱图和短时傅里叶变换（STFT） · 频率混叠（Aliasing） · 零填充不会提高分辨率 · 步骤 1：从零开始构建 DFT"
      },
      {
        ...
        "keywords": "图：节点和边 · 邻接矩阵 · 度数 · BFS 和 DFS · 图拉普拉斯矩阵 · 消息传递 · 步骤 1：从零开始编写图类```python · 步骤 2：BFS 和 DFS```python · 第三步：连通分量和拉普拉斯矩阵的特征值```python · 步骤 4：谱聚类```python · 步骤 5：消息传递```python · numpy 谱分析```python"
      },
      {
        ...
        "keywords": "随机游走 · 马尔可夫链 · 与语言模型的联系 · 布朗运动 · Langevin 动力学 · 人工智能中的随机过程 · 步骤 1：随机行走模拟器```python · 步骤 2：马尔可夫链```python · 步骤 3：朗之万动力学```python · 步骤 4：Metropolis-Hastings```python · 与实际框架的联系 · 验证马尔可夫链的收敛性```python"
      }
    ]
  },
  {
    ...
    "lessons": [
      ...
      },
      {
        ...
        "keywords": "模型 · 代价函数（均方误差） · 梯度下降 · 正规方程（闭式解） · 多元线性回归 · 多项式回归 · R 平方分数 · 正则化预览（岭回归） · 第 1 步：生成样本数据```python"
      },
      {
        ...
        "keywords": "为什么线性回归不适合分类 · Sigmoid 函数 · 逻辑回归 = 线性模型 + Sigmoid · 二元交叉熵损失 · 逻辑回归的梯度下降 · 使用 Softmax 进行多分类 · 评估指标 · 第 1 步：Sigmoid 函数与数据生成```python"
      },
      {
      ...
      },
      {
        ...
        "keywords": "K近邻的工作原理 · 选择 K · 加权KNN · 维度灾难 · KD 树：快速最近邻搜索 · 球树：适合中等维度 · 懒惰学习 vs 懒惰学习 · 回归中的 KNN · 第一步：距离函数 · 步骤 4：特征缩放"
      },
      {
        ...
        "keywords": "聚类：将相似的事物分组在一起 · 选择 K · DBSCAN：基于密度的聚类 · 层次聚类 · 高斯混合模型（GMM） · 何时使用哪种方法 · 使用聚类进行异常检测 · 步骤 1：从零开始实现 K-Means```python"
      },
      {
        ...
        "keywords": "特征管道```mermaid · 类别特征 · 文本特征 · 特征交互 · 特征选择 · 第一步：从零开始实现数值转换```python"
      },
      {
        ...
        "keywords": "训练集、验证集、测试集```mermaid · K折交叉验证 · 分类指标 · 回归指标 · 学习曲线 · 验证曲线 · 常见评估错误 · 第一步：训练/验证/测试划分```python"
      },
      {
        ...
        "keywords": "偏差：系统误差 · 模型复杂度与误差```mermaid · 正则化作为偏差-方差控制 · 双降：现代视角 · 诊断你的模型```mermaid · 实用策略 · 集成方法和方差减少 · 学习曲线 · 如何生成学习曲线 · 步骤 1：从已知函数生成合成数据 · 步骤 3：计算 Bias^2，方差分解 · 第 4 步：学习曲线 · 步骤5：正则化扫描 · 验证曲线：扫过模型复杂度```python · 学习曲线：遍历训练集大小```python · 带正则化扫描的交叉验证```python · 综合应用：完整的诊断工作流程"
      },
      {
        ...
        "keywords": "为什么集成有效 · Bagging（Bootstrap Aggregating） · 提升（序列错误修正） · AdaBoost · 梯度提升 · XGBoost：为何在表格数据上占主导地位 · Stacking（元学习） · 投票 · 步骤 1：决策桩（基学习器） · 何时使用每种方法 · 表格数据的生产堆栈"
      },
      {
        ...
        "keywords": "参数 vs 超参数 · 网格搜索 · 随机搜索 · 贝叶斯优化 · 早期停止（Early Stopping） · 学习率调度器（Learning Rate Schedulers） · 超参数重要性 · 实用策略```mermaid · 交叉验证集成 · 实用技巧 · 第一步：从零开始实现网格搜索 · 第四步：比较所有方法 · 实践中的 Optuna · 使用 Pruning 的 Optuna · sklearn 内置的调参器 · 超参数调优的常见错误"
      },
      {
        ...
        "keywords": "管道是什么 · 数据泄露：沉默的杀手 · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · Data Leakage: The Silent Killer · sklearn 管道 · ColumnTransformer：为不同列使用不同的管道 · 实验跟踪 · 使用 DVC 进行数据版本管理 · 可复现的实验 · 常见管道错误 · 第一步：自定义转换器```python · 步骤 4：使用 sklearn 的完整生产流水线"
      },
      {
        "name": "Naive Bayes",
        "status": "complete",
        ...
        "keywords": "贝叶斯定理（快速回顾） · 朴素独立性假设 · 为什么它仍然有效 · 数学步骤详解 · 三种变体 · 何时使用每种变体 · 拉普拉斯平滑 · 对数空间计算 · 朴素贝叶斯与逻辑回归 · 分类流程```mermaid · MultinomialNB · GaussianNB · 示例：文本分类 · 示例：连续特征 · 预测速度 · 使用朴素贝叶斯的 TF-IDF · 短文本的 BernoulliNB · 校准 NB 概率 · 常见陷阱 · 朴素贝叶斯失败的情况"
      },
      {
        ...
        "keywords": "使时间序列不同的因素 · 时间序列的组成部分 · 稳定性 · 自相关 · 滞后特征：将时间序列转换为监督学习 · 前向验证 · ARIMA直觉 · 何时使用什么 · 预测范围和策略 · 时间序列中的常见错误 · 滞后特征创建器```python · 走向前交叉验证```python · 简单的自回归模型 · 稳态检查 · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · Stationarity Check · 自相关性```python · sklearn 时间序列分割 · 滚动特征 · 你必须超越的基线 · 实用技巧"
      },
      {
        ...
        "keywords": "异常的类型 · 监督与无监督：权衡 · Z-Score 方法 · IQR 方法 · 隔离森林 · 局部异常因子 (LOF) · 对比 · 评估挑战 · Z 分数检测器```python · IQR 检测器```python · 演示场景 · sklearn 污染参数 · One-Class SVM · 自编码器方法（预览） · 集成异常检测 · 生产环境考虑 · 选择阈值 · 扩展到生产环境"
      },
      {
        ...
        "keywords": "为什么准确率失败 · 更好的指标 · 不平衡数据管道```mermaid · 类别权重 · 阈值调节 · 成本敏感学习 · 决策流程图```mermaid · 步骤 1：生成一个不平衡的数据集```python"
      },
      {
        ...
        "keywords": "特征选择的三类 · 方差阈值 · 互信息 · L1（Lasso）正则化 · 基于树的特征重要性 · 置换重要性 · 对比表格 · 决策流程图```mermaid · 步骤 1：生成具有已知特征结构的合成数据```python · 步骤 2：方差阈值```python"
      }
    ]
  },
  {
    ...
    "lessons": [
      {
        ...
        "keywords": "一个神经元，一个决策 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 决策边界 · 学习规则 · XOR 问题 · 第一步：Perceptron 类```python · 步骤 4：用两层解决 XOR · 第5步：训练一个两层网络"
      },
      {
        ...
        "keywords": "层：输入层、隐藏层、输出层 · 神经元和激活函数 · 前向传播：数据如何流动 · 矩阵维度 · 普适逼近定理 · 步骤 1：Sigmoid 激活函数```python · 第二步：层类 · 步骤3：网络类 · 第4步：使用手动调整的权重进行异或（XOR） · 步骤 5：圆分类"
      },
      {
        ...
        "keywords": "应用于网络的链式法则 · 计算图 · 前向与反向```mermaid · 梯度在网络中的流动 · 消失的梯度 · 为两层网络推导梯度 · 为两层网络推导梯度 · 第一步：值节点 · 步骤 2：具有反向函数的操作 · 步骤 3：Sigmoid 和 Loss```python · 步骤 4：反向传播 · 步骤 5：层和网络```python · 步骤6：在XOR上进行训练```python · 第 7 步：圆分类"
      },
      {
        ...
        "keywords": "为什么非线性是必要的 · Sigmoid · Tanh · ReLU：突破性进展 · Leaky ReLU · GELU：现代默认激活函数 · Swish / SiLU · Softmax：输出激活函数 · 形状比较```mermaid · 第一步：实现所有激活函数及其导数"
      },
      {
        ...
        "keywords": "均方误差（MSE） · 交叉熵损失 · 为什么 MSE 在分类任务中表现不佳```mermaid · 标签平滑 · Label Smoothing · 标签平滑 · Label Smoothing · 标签平滑 · Label Smoothing · 标签平滑 · Label Smoothing · 标签平滑 · Label Smoothing · 标签平滑 · Label Smoothing · 标签平滑 · Label Smoothing · 标签平滑 · Label Smoothing · 标签平滑 · Label Smoothing · 标签平滑 · Label Smoothing · 标签平滑 · Label Smoothing · 标签平滑 · Label Smoothing · 标签平滑 · Label Smoothing · 标签平滑 · Label Smoothing · 标签平滑 · Label Smoothing · 对比损失 · 焦点损失 · 损失函数决策树```mermaid · 第一步：MSE 及其梯度```python · 步骤 4：标签平滑（Label Smoothing）```python"
      },
      {
        ...
        "keywords": "随机梯度下降（SGD） · 动量 · RMSProp · Adam：动量 + RMSProp · AdamW：正确实现权重衰减 · 学习率：最重要的超参数```mermaid · 优化器对比```mermaid · 第一步：普通 SGD```python"
      },
      {
        ...
        "keywords": "过拟合光谱 · 权重衰减（L2正则化） · 批量归一化 · 层归一化 · RMSNorm · 归一化比较```mermaid · 早停法 · 何时应用什么```mermaid · 第一步：Dropout（训练和评估模式）```python"
      },
      {
        ...
        "keywords": "对称性问题 · 通过层的方差传播 · Xavier/Glorot 初始化 · Kaiming/He 初始化 · Transformer 初始化 · 第一步：初始化策略"
      },
      {
        ...
        "keywords": "常数学习率 · 步长衰减（Step Decay） · 余弦退火（Cosine Annealing） · 预热（Warmup）：为什么你从很小的学习率开始 · 线性预热 + 余弦衰减 · 1cycle 策略 · 调度形状```mermaid · 第一步：安排函数"
      },
      {
        ...
        "keywords": "Module抽象 · Sequential容器 · 训练模式 vs 评估模式 · 优化器 · DataLoader · 框架架构 · 步骤 1：模块基类"
      },
      {
        ...
        "keywords": "为什么 PyTorch 赢了 · 张量 · nn.Module · 损失函数与优化器 · 训练循环 · 数据集和数据加载器 · GPU 训练 · 步骤 1：从原始文件加载 MNIST · 步骤 3：训练循环 · 步骤 4：将所有部分连接起来```python · 快速比较：小框架 vs PyTorch · 保存和加载模型```python · 学习率调度```python"
      },
      {
        ...
        "keywords": "JAX 的哲学 · jax.numpy：熟悉的表面 · jax.grad：函数式自动微分 · jit: 编译到 XLA```python · vmap：自动向量化 · pmap：跨设备的数据并行```python · Pytrees：通用数据结构 · 功能性与面向对象 · JAX 生态系统 · JAX 中的随机数 · 第一步：设置和数据 · 第三步：前向传播```python · 步骤 4：JIT 编译的训练步骤```python · 第 5 步：训练循环```python · Flax：Google 标准 · Equinox：Pythonic 的替代方案 · Optax：可组合的优化器 · Optax：可组合的优化器"
      },
      {
        ...
        "keywords": "调试心态 · 症状 2：损失下降但模型表现差 · 症状 3：损失中出现 NaN 或 Inf · 技术 1：梯度检查 · 技术 3：梯度流动可视化 · 技巧 5：学习率查找器 · 常见 PyTorch 错误 · 主要调试表 · 第一步：NetworkDebugger 类 · PyTorch 内置工具```python"
      }
    ]
  },
  {
    ...
    "lessons": [
      {
        ...
        "keywords": "整个预处理流程一览 · 一个像素是一个样本，而不是一个方块 · 为什么是三个通道 · 两种布局惯例：HWC 和 CHW · 颜色空间及其存在的原因 · 步骤 1：加载图像并检查其形状 · 步骤 2：分离通道并重新排列布局 · 步骤3：灰度和HSV转换 · 第 4 步：归一化、标准化并反转它 · 步骤 5：使用三种插值方法进行调整大小"
      },
      {
        ...
        "keywords": "一个核，滑动 · 输出尺寸公式 · Padding · 步长 · im2col 技巧 · 接收域 · 步骤 1：填充数组 · 步骤 2：使用嵌套循环的二维卷积 · 步骤 3：使用手工设计的核进行验证 · 第4步：im2col · 步骤 5：通过 im2col + matmul 实现快速卷积 · 第6步：一组手工设计的内核"
      },
      {
        ...
        "keywords": "改变视觉的四个想法```mermaid · LeNet-5 (1998) · AlexNet（2012） · VGG（2014） · Inception（2014 年，同年） · 退化问题 · ResNet (2015) · 为何残差连接在视觉以外的领域也很重要 · 步骤 1：LeNet-5 · 步骤 2：一个 VGG 模块 · 步骤 3：ResNet 基本块 · 步骤 4：一个微型 ResNet · 步骤5：比较参数与特征的效率"
      },
      {
        ...
        "keywords": "分类流水线```mermaid · 交叉熵、logits和softmax · 为什么增强有效 · Mixup 和 Cutmix · 标签平滑 · 超越准确率的评估 · 第一步：确定性合成数据集 · 第二步：归一化和增强 · 步骤3：Mixup · 第4步：训练循环 · 第五步：将它们组合在一起 · 第六步：阅读混淆矩阵"
      },
      {
        ...
        "keywords": "特征提取与微调 · 为什么冻结任何层都能起作用 · 有区别的学习率 · BatchNorm 的问题 · Head 设计 · 按层衰减的学习率 · 需要评估的内容 · 步骤 1：加载预训练的主干网络并检查它```python · 步骤 2：特征提取 — 冻结所有参数，替换头部分```python · 步骤 3：判别式微调 · 第 4 步：BatchNorm 处理 · 步骤 5：一个最小的端到端微调循环```python · 第 6 步：渐进解冻"
      },
      {
        ...
        "keywords": "作为密集预测的检测 · 为什么使用网格和锚点 · 解码预测 · IoU · 非极大值抑制 · 损失函数 · 检测指标 · 第一步：IoU · 步骤 2：非极大值抑制```python · 步骤 3：Box 编码和解码 · 步骤 4：一个最小的 YOLO 头 · 步骤 5：真实值分配 · 第六步：三种损失```python · 第 7 步：推理流程"
      },
      {
        ...
        "keywords": "语义 vs 实例 vs 全景```mermaid · U-Net 的结构```mermaid · 反转卷积与双线性上采样 · 像素网格上的交叉熵 · Dice 损失及其必要性 · 评估指标 · 输入分辨率的权衡 · 第一步：编码器模块 · 步骤 2：下采样和上采样模块```python · 第3步：U-Net```python · 第 4 步：损失函数```python · 步骤 5：IoU 指标```python · 第 6 步：用于端到端验证的合成数据集 · 第7步：训练循环```python"
      },
      {
        ...
        "keywords": "架构```mermaid · 为什么使用RoIAlign而不是RoIPool · 用一段话描述 RPN · 掩码头部 · 损失函数 · 输出格式 · 步骤 1：从零开始实现 RoIAlign · 步骤 2：与 torchvision 的 RoIAlign 进行比较```python · 步骤 3：加载预训练的 Mask R-CNN```python · 步骤 4：运行推理```python · 步骤 6：冻结不需要训练的部分"
      },
      {
        ...
        "keywords": "两个网络```mermaid · 游戏 · 非饱和损失 · DCGAN 架构规则 · 失败模式及其特征```mermaid · 评估 · 步骤 1：生成器 · 步骤 2：判别器 · 步骤 3：训练步骤 · 步骤 4：在合成形状上进行完整的训练循环```python · 第 5 步：采样```python · 步骤 6：谱归一化"
      },
      {
        ...
        "keywords": "前向过程 · 闭式跳跃 · 逆过程 · 采样器（DDPM） · 为什么是 1000 步 · DDIM：20 倍更快的采样 · 时间条件 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 第一步：噪声计划 · 步骤 2：前向扩散（q_sample）```python · 步骤3：一个小型的时间条件U-Net```python · 步骤 4：训练循环```python · 步骤 5：采样器（DDPM）```python · 第6步：DDIM采样器（确定性，约20倍更快）```python"
      },
      {
        ...
        "keywords": "管道```mermaid · 无分类器引导（CFG） · 潜在空间几何 · U-Net 架构 · LoRA 微调 · 你会看到的调度器 · 第一步：文本到图像```python · 步骤 2：更换调度器```python · 步骤 3：图像到图像```python · 步骤 4：修复绘画```python · 步骤 5：LoRA 加载```python · 步骤 6：LoRA 训练（草图）"
      },
      {
        ...
        "keywords": "三种架构家族```mermaid · 3D卷积 · 时空变换器 · 帧采样 · 评估 · 你将遇到的数据集 · 第一步：帧采样器 · 步骤 2：2D+池化基线 · 第3步：一种I3D风格的膨胀3D卷积 · 步骤 4：分解的（2+1）D 卷积"
      },
      {
        ...
        "keywords": "点云 · PointNet 架构```mermaid · 神经辐射场（NeRFs） · NeRF 中的位置编码 · 体素渲染``` · 什么取代了 NeRF · 数据集和基准 · 第一步：PointNet 分类器```python · 步骤 2：位置编码```python · 步骤 3：Tiny NeRF MLP```python · 步骤 4：沿光线进行体积渲染```python"
      },
      {
        ...
        "keywords": "流程图```mermaid · 补丁嵌入 · 类别标记 · 位置嵌入 · Transformer 编码器块 · 为什么使用pre-LN · 补丁大小的权衡 · DeiT在ImageNet-1k上训练ViT的配方 · Swin与ConvNeXt的对比 · MAE预训练 · 第一步：补丁嵌入```python · 步骤 2：Transformer 块 · 第三步：ViT```python · 步骤 4：合理性检查 —— 单张图像推理```python"
      },
      {
        ...
        "keywords": "三个预算```mermaid · 测量规范 · FLOPs 作为代理 · 量化（一段话） · 剪枝与知识蒸馏 · 推理运行时 · 边缘架构选择器 · 步骤 1：正确测量延迟```python · 步骤 2：参数和 FLOP 数量```python · 第 3 步：训练后静态量化```python · 步骤 4：导出到 ONNX```python · 步骤 5：基准测试并比较制度```python"
      },
      {
        ...
        "keywords": "流水线```mermaid · 使用 Pydantic 的数据契约 · 延迟去向 · 故障模式 · 批处理 · 第一步：数据契约```python · 步骤 2：一个最小的 Pipeline 类```python · 第三步：连接一个探测器和一个分类器```python · 步骤 5：对管道进行基准测试```python"
      },
      {
        ...
        "keywords": "三个家族```mermaid · 教师-学生（DINO） · 掩码重建（MAE） · 为什么是 75% 而不是 15% · 线性探针评估 · 第一步：双视图增强管道 · 第一步：双视图增强管道```python · 步骤 2：InfoNCE 损失```python · 步骤 3：对 InfoNCE 进行合理性检查```python · 步骤 4：MAE 风格的掩码```python"
      },
      {
        ...
        "keywords": "双塔结构```mermaid · 目标 · SigLIP：更好的损失函数 · 零样本分类 · 2026年CLIP风格模型的应用场景 · 第一步：一个微小的双塔模型 · 步骤 2：对比损失```python · 步骤 3：零样本分类器```python · 步骤 4：合理性检查```python"
      },
      {
        ...
        "keywords": "经典流程```mermaid · 一段话中的 CTC · 现代端到端模型 · 布局解析 · 评估指标 · 第一步：CTC 损失 + 贪婪解码器```python · 步骤 2：Tiny CRNN 识别器 · 步骤3：合成OCR · 步骤4：训练草图```python"
      },
      {
        ...
        "keywords": "一瞥检索```mermaid · 正式定义三元组损失``` · 余弦相似性 vs L2 距离 · Recall@K · 用一段话介绍 FAISS · 实例级与类别级检索 · 步骤 1：三元组损失```python · 步骤 2：半硬挖掘 · 步骤 3：Recall@K```python · 步骤 4：整合起来```python"
      },
      {
